...
import { ConnectivityStatus, EducationLevel, MinMax, Setter, Translation } from 'src/@types'
...
export default function ConnectivityTableFilters({
  ...
  const handleMaxBudget = setFilterBudget
    ? (max: string) => handleFilter(() => setFilterBudget.max(max))
    : null

  const handleMinBudget = setFilterBudget
    ? (min: string) => handleFilter(() => setFilterBudget.min(min))
    : null
  ...
  const handleResetFilter = () => {
    ...
    if (setFilterBudget?.max) setFilterBudget.max('')
    if (setFilterBudget?.min) setFilterBudget.min('')
  }
